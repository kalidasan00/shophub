...
type Props = {
  ...
  stats: { shopCount: number; productCount: number }
  loadError: boolean
}

export default function HomeClient({ shops, dealProducts, topRatedProducts, stats, loadError }: Props) {
  ...
  return (
    <div style={{ fontFamily: font.family, backgroundColor: '#F8F9FF', minHeight: '100vh' }}>
      ...
      {/* ── ERROR STATE ── */}
      {loadError && (
        <div style={{ maxWidth: '480px', margin: '1rem auto 0', padding: '0 1.25rem' }}>
          <div style={{
            background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: '12px',
            padding: '12px 14px', fontSize: '13px', color: '#991B1B', fontFamily: font.family,
          }}>
            We couldn't load some homepage content. Please refresh, or check back shortly.
          </div>
        </div>
      )}
          ...
          <div style={{ maxWidth: '480px', margin: '0 auto', padding: '0 1.25rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem' }}>
              ...
              <div style={{ display: 'flex', gap: '5px' }}>
                <button onClick={() => scrollDeals('left')} aria-label="Scroll deals left" style={{ width: '28px', height: '28px', borderRadius: '50%', border: '1px solid #EBEBF5', background: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
                  <ChevronLeft size={14} color="#0F0F1A" />
                </button>
                <button onClick={() => scrollDeals('right')} aria-label="Scroll deals right" style={{ width: '28px', height: '28px', borderRadius: '50%', border: '1px solid #EBEBF5', background: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
