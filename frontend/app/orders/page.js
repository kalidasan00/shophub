  ...
  return (
    <div style={{ maxWidth: '900px', margin: '0 auto', padding: '2rem 1.5rem 5rem', fontFamily: font.family }}>
      ...
      <div style={{ marginBottom: '1.75rem' }}>
        <h1 style={{ fontSize: 'clamp(1.5rem, 3vw, 2rem)', fontWeight: 700, color: colors.dark, marginBottom: '6px' }}>
          My Orders
        </h1>
      ...
      {justPlaced && (
        <div style={{
          backgroundColor: '#F0FDF4', border: '1px solid #BBF7D0', borderRadius: radius.lg,
      ...
      {!loading && error && (
        <div style={{ textAlign: 'center', padding: '4rem 1rem' }}>
          ...
          <h3 style={{ fontWeight: 600, fontSize: '1rem', color: colors.dark, marginBottom: '0.375rem' }}>
      ...
      {!loading && !error && orders.length === 0 && (
        <div style={{ textAlign: 'center', padding: '4rem 1rem' }}>
          ...
          <h3 style={{ fontWeight: 600, fontSize: '1.125rem', color: colors.dark, marginBottom: '0.5rem' }}>
            No orders yet
          </h3>
          <p style={{ fontSize: font.base, color: colors.muted, marginBottom: '1.5rem' }}>
            When you place an order, it'll show up here.
          </p>
          <Link
            href="/shops"
            style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', backgroundColor: colors.dark, color: colors.white, textDecoration: 'none', padding: '12px 24px', borderRadius: radius.md, fontSize: font.base, fontWeight: 500 }}
          >
            Browse Shops
          </Link>
        </div>
    ...
    <div style={{ backgroundColor: colors.white, borderRadius: radius.xxl, border: `1px solid ${colors.border}`, boxShadow: shadow.card, overflow: 'hidden' }}>
      ...
      <button
        ...
        style={{
          ...
          gap: '1rem', padding: '1.25rem', background: 'none', border: 'none',
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', flex: 1, minWidth: 0 }}>
          ...
          <div style={{ display: 'flex', flexShrink: 0 }}>
            {previewItems.map((item, idx) => (
              <div
                ...
                style={{
                  ...
                  fontSize: '18px', marginLeft: idx === 0 ? 0 : '-12px',
            ...
            {extraCount > 0 && (
              <div style={{
                width: '40px', height: '40px', backgroundColor: colors.surface,
                border: `2px solid ${colors.white}`, borderRadius: radius.md,
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                fontSize: font.xs, fontWeight: 600, color: colors.muted, marginLeft: '-12px',
              }}>
                +{extraCount}
              </div>
            )}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', flexShrink: 0 }}>
          <span style={{
            ...
            padding: '5px 12px', borderRadius: radius.full, whiteSpace: 'nowrap',
          }}>
            {status.label}
          </span>
          ...
          <svg
            width="18" height="18" fill="none" stroke={colors.muted} viewBox="0 0 24 24"
      ...
      {expanded && (
        <div style={{ borderTop: `1px solid ${colors.border}`, padding: '1.25rem' }}>
          ...
          <div style={{ backgroundColor: colors.surface, borderRadius: radius.lg, padding: '0.875rem 1rem', marginBottom: '1.25rem' }}>
            <p style={{ fontSize: font.xs, fontWeight: 600, color: colors.muted, marginBottom: '4px', textTransform: 'uppercase', letterSpacing: '0.02em' }}>
              Shipping Address
            </p>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', fontSize: font.sm }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              ...
              <span style={{ color: order.shippingCost === 0 ? colors.green : colors.dark }}>
          ...
          <p style={{ fontSize: font.xs, color: colors.muted, marginTop: '1rem', marginBottom: 0 }}>
            ...
            {' · '}
            Payment status: <span style={{ fontWeight: 600, color: order.paymentStatus === 'paid' ? colors.green : colors.muted }}>{order.paymentStatus}</span>
          </p>
        </div>
      )}
    </div>
